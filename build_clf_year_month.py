...
def data_batch_loader(data_name, batch_size=100, mode='train'):
    test_label = sorted(
        [
            file_name.split('.')[0].split('_')[1].strip() 
                for file_name in os.listdir('./vects/'+data_name+'/') 
                    if 'year' in file_name]
    )
    if mode == 'valid':
        valid_label = test_label[-3]
    test_label = test_label[-2]  # the latest year

    batch_data = {
        'data': [], 'label': [], 'year_label':[], 'month_label': []
    }
    ...
    with open('./data/'+data_name + '_year_month.tsv') as datafile:
        datafile.readline()
        for line in datafile:
            infos = line.strip().split('\t')
            if mode == 'train' and infos[2] == test_label:
                continue
            if mode == 'test':
                if infos[1] != test_label:
                    continue
            if mode == 'valid':
                if infos[1] != valid_label:
                    continue
            all_data.append(infos[0])
            all_label.append(infos[2])
            all_year_label.append(infos[1])
            all_month_label.append(infos[3])
    ...
    if mode == 'train':  # over sampling
        ...
        all_month_label = [all_month_label[item[0]] for item in indices]
        all_year_label = [all_year_label[item[0]] for item in indices]
        all_data = [all_data[item[0]] for item in indices]

    for item in zip(all_data, all_label, all_year_label, all_month_label):
        batch_data['data'].append(item[0])
        batch_data['label'].append(item[1])
        batch_data['year_label'].append(item[2])
        batch_data['month_label'].append(item[3])
...
def create_domain_vects(data_name, mode='year'):
    ...
    for domain_name in domain_docs:
        ...
        pickle.dump(
            da_vect,
            open('./vects/'+data_name+'/'+mode+'_'+str(domain_name)+'.pkl', 'wb')
            )
    return list(domain_docs.keys())


def create_domain_clfs(data_name):
    domains = {'year': [], 'month': []}
    sum_fea_size = 0
    fea_size = {'year': dict(), 'month': dict()}

    # get feature size of each vectorizer:
    print('\t\tGet domain information.....')
    for file_name in os.listdir('./vects/' + data_name + '/'):
        with open('./vects/' + data_name + '/' + file_name, 'rb') as vect_pkl_f:
            vect_pkl = pickle.load(vect_pkl_f)
            cur_domain = file_name.split('.')[0].split('_')[1].strip()
            sum_fea_size += len(vect_pkl.vocabulary_)

            if 'year' in file_name:
                domains['year'].append(cur_domain)
                fea_size['year'][cur_domain] = len(vect_pkl.vocabulary_)
            else:
                domains['month'].append(cur_domain)
                fea_size['month'][cur_domain] = len(vect_pkl.vocabulary_)
    ...
    # load the data
    batch_size = 1000
    train_iter = data_batch_loader(data_name, batch_size)

    # load the general vect
    general_vect = pickle.load(open('./vects/' + data_name + '/year_general.pkl', 'rb'))
    print('\t\tBacth fit............')
    batch_count = 0
    for train_batch in train_iter:
        ...
        train_data[:, :fea_size['year']['general']] = general_vect.transform(train_batch['data'])

        start_idx = fea_size['year']['general']

        for mode in ['year', 'month']:
            for domain_name in domains[mode]:
                if domain_name == 'general':
                    continue
                with open('./vects/' + data_name + '/' + mode + '_' + str(domain_name) + '.pkl', 'rb') as vect_pkl_f:
                    vect_pkl = pickle.load(vect_pkl_f)
                    transformed_data = vect_pkl.transform(train_batch['data'])
                    
                    for label_idx in range(len(train_batch[mode+'_label'])):
                        if train_batch[mode+'_label'][label_idx] == domain_name:
                            train_data[label_idx, start_idx:start_idx+fea_size[mode][domain_name]] = transformed_data[label_idx, :]
                start_idx += fea_size[mode][domain_name]  # update the start index
        # partial training
    ...
    with open('./clfs/'+data_name+'.pkl', 'wb') as clf_file:
        pickle.dump(
            clf,
            clf_file
        )
...
def run_exp(data_name, create_vects=True, create_clfs=True):
    print('Working on: ' + data_name + '..............................')
    if not os.path.exists('./vects/'+data_name):
        os.mkdir('./vects/'+data_name)

    if create_vects:
        print('\tCreating monthly vects.........')
        create_domain_vects(data_name, mode='month')
        print('\tCreating yearly vects.........')
        year_domains = sorted(create_domain_vects(data_name))
        print(year_domains)
    ...
    gen_vect = pickle.load(open('./vects/'+data_name+'/year_general.pkl', 'rb'))
    ...
    with open('results_year_month.txt', 'a') as result_file:
...
if __name__ == '__main__':
    ...
    p = Pool(2)
    p.map(run_exp, data_list)
#    for data in data_list:
#        run_exp(data, create_vects=False, create_clfs=False)
